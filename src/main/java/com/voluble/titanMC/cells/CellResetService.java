...
public final class CellResetService {
	...
	private final class Scan extends BukkitRunnable {
		private final CellResetJob job;private final CellDefinition cell;private final World world;private final List<Chunk> chunks;private final List<Inventory> inventories=new ArrayList<>();private final Set<Inventory> seen=Collections.newSetFromMap(new IdentityHashMap<>());private final List<ItemStack> items=new ArrayList<>();private int x,y,z;
		private Scan(CellResetJob job,CellDefinition cell,World world,List<Chunk> chunks){this.job=job;this.cell=cell;this.world=world;this.chunks=chunks;x=cell.cuboid().minX;y=cell.cuboid().minY;z=cell.cuboid().minZ;}
		@Override public void run(){int done=0;while(done++<BLOCKS_PER_TICK){var block=world.getBlockAt(x,y,z);if(block.getState() instanceof Container container){Inventory inventory=container.getInventory();if(seen.add(inventory)){inventories.add(inventory);if(job.phase()==CellResetJob.Phase.COLLECTING)for(ItemStack item:inventory.getContents())if(item!=null&&!item.getType().isAir())items.add(item.clone());}}if(!advance()){cancel();prepareOrClear();return;}}}
		private boolean advance(){if(++z<=cell.cuboid().maxZ)return true;z=cell.cuboid().minZ;if(++x<=cell.cuboid().maxX)return true;x=cell.cuboid().minX;return ++y<=cell.cuboid().maxY;}
		private void prepareOrClear(){if(job.phase()==CellResetJob.Phase.PREPARED){clear(job.recoveryLotId());return;}CellLease lease=cells.lease(job.cellId());if(lease==null){release();return;}List<byte[]> serialized=items.stream().map(ItemStack::serializeAsBytes).toList();cells.storage().createRecoveryLot(lease,serialized).whenComplete((lot,error)->Bukkit.getScheduler().runTask(plugin,()->{if(error!=null){plugin.getLogger().severe("Failed to prepare reset for "+cell.id()+": "+error.getMessage());release();return;}cells.markPrepared(job,lot);clear(lot);}));}
